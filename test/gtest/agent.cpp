...
public:
  ...
  ~AgentHelper() {
    /* We must release nixlAgent first, as it calls cleanup functions in gmock_engine,
       which must stay alive during the process. */
    agent_.reset();
  }
...
protected:
  ...
  void SetUp() override {
    agent_helper_ = std::make_unique<AgentHelper>(local_agent_name, DRAM_SEG);
...
protected:
  ...
  void SetUp() override {
    local_agent_helper_ = std::make_unique<AgentHelper>(local_agent_name, DRAM_SEG);
    remote_agent_helper_ = std::make_unique<AgentHelper>(remote_agent_name, DRAM_SEG);
...
protected:
  ...
  void SetUp() override {
    agent_helper_ = std::make_unique<AgentHelper>(local_agent_name, GetParam());
    agent_ = agent_helper_->GetAgent();
  }
};
